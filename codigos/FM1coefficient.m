function [coefficient,auxface]=FM1coefficient(F,y,kmap)
global inedge bedge coord elem centelem numcase
Klef=zeros(3,3);
Krel=zeros(3,3);
R=[0 1 0; -1 0 0;0 0 0];
...
for ifacont=1:size(bedge,1)
    ...
    for i=auxvetor
        if i~=auxvetor(length(auxvetor))
            ...
            if ((sign(auxquadrant1(1,3))==sign(auxquadrant2(1,3)) && ...
                    (abs(auxquadrant1(1,3))>=paramlivre || abs(auxquadrant2(1,3))>=paramlivre))...
    ...
    if (ksii==1e30 && ksij==1e30) || (ksii>1e10 && ksij>1e10)
        ...
        [ksii,ksij,aux11,aux12,auxy]=Faroundfacelement(F,y,lef,ve2,klef,kmap,paramlivre);
        ...
        coefficient(1,5,ifacont)=norm((auxy(aux11,:)-centelem(lef,:))*...
            coefficient(1,1,ifacont)+ (auxy(aux12,:)-centelem(lef,:))*coefficient(1,2,ifacont)-ve2');
        ...
        % verificando a identidade
        coefficient(1,5,ifacont)=norm((y(aux11,:)-centelem(lef,:))*coefficient(1,1,ifacont)+ ...
            (y(aux12,:)-centelem(lef,:))*coefficient(1,2,ifacont)-ve2');
        if abs(coefficient(1,5,ifacont))<1e-10
        else
    ...
    [auxface]=Fcalfacelement(lef,ifacont,aux11,aux12,bedge,inedge,auxface,1,2);
...
for iface=1:size(inedge,1)
    ...
    [auxface]=Fcalfacelement(lef,iface+size(bedge,1),aux11,aux12,bedge,inedge,auxface,1,2);
    ...
    for ii=auxvetorel
        if ii~=auxvetorel(length(auxvetorel))
            ...
            % Estes condições evitam que o acos seja numero complexo.
            if (dot(vetorj,vetor12)/(norm(vetorj)*norm(vetor12)))>1 &&...
                    abs(1-dot(vetorj,vetor12)/(norm(vetorj)*norm(vetor12)))<1e-10
                % calculo do theta2
                thetarel2=acos(1);
            else
            ...
            % evita que aparição de numeros complexos
            if (dot(vetori,vetor12)/(norm(vetori)*norm(vetor12)))>1 && ...
                    abs(1-dot(vetori,vetor12)/(norm(vetori)*norm(vetor12)))<1e-10
                % calculo do theta1
                thetarel1=acos(1);
            else
    ...
    if (ksii==1e30 && ksij==1e30)|| (ksii>1e10 && ksij>1e10)
        ...
        [ksii,ksij,auxi,auxj,auxy]=Faroundfacelement(F,y,rel,vetor12,krel,kmap,paramlivre);
        % atribuindo valores a os coeficientes
    ...
    [auxface]=Fcalfacelement(rel,iface+size(bedge,1),auxi,auxj,bedge,inedge,auxface,3,4);
