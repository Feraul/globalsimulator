function [pressurinterp]=Fpressureinterp(p,nflagface,nflagno,w,s,auxflag,parameter,weightDMP,mobility)
global inedge coord esurn1 esurn2 bedge bcflag pmethod phasekey
...
if strcmp(pmethod,'nlfvdmp')
    ...
    for ifacont=1:size(bedge,1)
        ...
        elseif bedge(ifacont,5)==205
            ...
            pressurinterp(ifacont,1)= nflag(ifacont,2);
        end
            ...
            if faceoposto<size(bedge,1) || faceoposto==size(bedge,1)
                ...
                if nflagface(faceoposto,1)<200
                    ...
                    if phasekey == 1
                        ...
                        %"mobonface" receivees the summation of water and oil
                        %mobilities (came from "IMPES" - Marcio's code modification)
                        mobonface = sum(mobility(faceoposto,:));
                    end  %End of IF
...
elseif strcmp(pmethod,'nlfvpp') || strcmp(pmethod,'mpfahd')
    ...
    for no=1:size(coord,1)
        ...
        if nflagno(no,1)>200
            auxvetor=1:esurn2(no+1)-esurn2(no);
            auxvetor1=auxvetor+esurn2(no);
            auxesurn1=esurn1(auxvetor1);
            
            weightpress= w(auxvetor1)'.*p(auxesurn1);
            
            if nflagno(no,1)==auxflag
                
                p1=s(no,1)+sum(weightpress);
            else
