function [M,I]=FassemblematrixlfvHP(parameter,nflagface,weightDMP,mobility)
...
for ifacont=1:size(bedge,1)
    ...
    lef=bedge(ifacont,3);
    % calcula a comprimento da face no contorno
    normcont=norm(coord(bedge(ifacont,1),:)-coord(bedge(ifacont,2),:));
    
    if bedge(ifacont,5)>200 % verifica o contorno de Neumann
        x=bcflag(:,1)==bedge(ifacont,5); % a quinta coluna da matriz 
        % bedge caracteriza o flag do face
        r=find(x==1);
        % Atribui o fluxo prescrito 
        I(lef)=I(lef)- normcont*bcflag(r,2);
    else
        % calcula a mobilidade na face em questão
        auxmobility=mobonface;
        % atribui-se as contribuições no elemento do contorno
        ...
        [M,I]=FtratmentcontourlfvHP(ifacelef1,parameter,nflagface,...
        ...
        [M,I]=FtratmentcontourlfvHP(ifacelef2,parameter,nflagface,...
...
for iface=1:size(inedge,1)
    ...
    %------------------------ somando 1 ----------------------------------%
    termo0=mobonface*norma*murel*parameter(1,1,ifactual);
    if ifacelef1<size(bedge,1) || ifacelef1==size(bedge,1)
        if nflagface(ifacelef1,1)<200
            ...
            mobil=sum(mobility(ifacelef1,:)); %mobility(ifacelef1); 
            ...
            if faceoposto<size(bedge,1) || faceoposto==size(bedge,1)
                if nflagface(faceoposto,1)<200
                    ...
                    mobilO=sum(mobility(faceoposto,:)); %mobility(faceoposto); 
    ...
    %--------------------------- somando 2 -------------------------------%
    termo0=mobonface*norma*murel*parameter(1,2,ifactual);
    if ifacelef2<size(bedge,1) || ifacelef2==size(bedge,1)
        if nflagface(ifacelef2,1)<200
            ...
            % calcula a mobilidade na face "ifacelef1"
            mobil=sum(mobility(ifacelef2,:)); %mobility(ifacelef2); 
            % verifica se a face oposto a "ifacelef1" pertence ao contorno
            ...
            if faceoposto<size(bedge,1) || faceoposto==size(bedge,1)
                if nflagface(faceoposto,1)<200
                    ...
                    mobilO=sum(mobility(faceoposto,:));
                    normcontopost=norm(coord(bedge(faceoposto,1),:)-coord(bedge(faceoposto,2),:));
    ...
    % somando 1
    termo0=mobonface*norma*mulef*parameter(2,1,ifactual);
    ...
    % somando 2
    termo0=mobonface*norma*mulef*parameter(2,2,ifactual);
