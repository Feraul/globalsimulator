function [flowrate,flowresult]=FflowratelfvLPEW(parameter,weightDMP,mobility,pinterp,p)
global inedge coord bedge bcflag centelem phasekey numcase;
 if numcase==31.2
        auxbcflag=bcflag;
        auxbcflag(2,2)=-1;
    else
...
for ifacont=1:size(bedge,1)
    ...
    if bedge(ifacont,5)>200
        ...
        r=find(x==1);
        flowrate(ifacont,1)=normcont*auxbcflag(r,2);
    else
        nolef1=parameter(1,3,ifacont);
        nolef2=parameter(1,4,ifacont);
        
        flowrate(ifacont,1)=  mobonface*normcont*((parameter(1,1,ifacont)+parameter(1,2,ifacont))*p(lef)-...
...
for iface=1:size(inedge,1)
    ...
    flowrate(iface+size(bedge,1),1)= mobonface*(murel*fluxesq-mulef*fluxdireit);
